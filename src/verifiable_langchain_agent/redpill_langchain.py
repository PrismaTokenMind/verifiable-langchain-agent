...
from langchain_core.messages import (
    ...
    SystemMessage,
    SystemMessageChunk,
    ToolMessage, InvalidToolCall, ToolCall,
)
...
DEFAULT_API_BASE = "https://api.red-pill.ai/v1/chat/completions"
...
def _convert_dict_to_message(_dict: Mapping[str, Any]) -> BaseMessage:
    ...
    if role == "user":
        ...
        # Parse tool calls if present in the message
        if raw_tool_calls := _dict.get("tool_calls"):
            additional_kwargs["tool_calls"] = raw_tool_calls
            for raw_tool_call in raw_tool_calls:
                try:
                    # Extract and parse arguments from JSON string to dictionary
                    function_info = raw_tool_call.get("function", {})
                    arguments = json.loads(function_info.get("arguments", "{}"))

                    # Construct ToolCall instance
                    tool_call = {
                        "name": function_info.get("name"),
                        "args": arguments,  # Ensure args is a dictionary
                        "id": raw_tool_call.get("id"),
                        "type": "tool_call"
                    }
                    tool_calls.append(tool_call)
                except Exception as e:
                    # Handle parsing errors by creating an invalid tool call entry
...
class ChatRedpill(BaseChatModel):

    """Redpill chat model integration.

    Setup:
        To use, you should have the environment variable``REDPILL_API_KEY`` set with
    your API KEY.
    ...
    Key init args — completion params:
        ...
        tools: Optional[List[Union[Dict[str, Any], Type[BaseModel], Callable, BaseTool]]]
            List of tool definitions to bind to this chat model.

    Key init args — client params:
        api_key: Optional[str]
            Redpill API key. If not passed in will be read from env var REDPILL_API_KEY.
    ...
    Tool calling:
        ...
        .. code-block:: python
            ...
            llm_with_tools = ChatRedpill(model='Redpill3-Turbo').bind_tools([get_current_weather])
    ...
    def validate_environment(cls, values: Dict) -> Any:
        values["redpill_api_base"] = get_from_dict_or_env(
            values,
            "redpill_api_base",
            "REDPILL_API_BASE",
            DEFAULT_API_BASE,
        )
        values["redpill_api_key"] = convert_to_secret_str(
            get_from_dict_or_env(
                values,
                ["redpill_api_key", "api_key"],
                "REDPILL_API_KEY",
            )
        )
        return values
    ...
    def _default_params(self) -> Dict[str, Any]:
        """Get the default parameters for calling Redpill API."""
        normal_params = {
            "model": self.model,
    ...
    async def _agenerate(
        ...
        **kwargs: Any,
        ...
        async with httpx.AsyncClient(
            ...
            response = await client.post(self.redpill_api_base, json=payload)
        ...
        **kwargs: Any,
        ...
        async with httpx.AsyncClient(
            ...
            async with aconnect_httpx_sse(
                client, "POST", self.redpill_api_base, json=payload
            ) as event_source:
